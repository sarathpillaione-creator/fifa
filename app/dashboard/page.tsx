...
export default function Dashboard() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const { data: mData } = await supabase.from('matches').select('*').gte('kickoff_utc', today.toISOString()).lt('kickoff_utc', tomorrowEnd.toISOString()).order('kickoff_utc', { ascending: true })
    ...
    <div className="min-h-screen bg-slate-50 p-6 text-black">
      ...
      {activeTab === 'predictions' && matches.map(m => (
        <div key={m.match_number} className="bg-white p-6 mb-4 border-2 border-black rounded shadow-sm">
          ...
          <input type="number" value={inputs[m.match_number]?.home || ''} onChange={(e) => setInputs({...inputs, [m.match_number]: {...inputs[m.match_number], home: e.target.value}})} className="border-2 border-black p-3 w-20 font-black text-lg" />
          <input type="number" value={inputs[m.match_number]?.away || ''} onChange={(e) => setInputs({...inputs, [m.match_number]: {...inputs[m.match_number], away: e.target.value}})} className="border-2 border-black p-3 w-20 font-black text-lg ml-2" />
