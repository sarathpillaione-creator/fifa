...
export default function Dashboard() {
  ...
  const savePred = async (m: any) => {
    ...
    if (!p.home || !p.away || !p.winner || !p.penalties) {
      alert("Please complete all fields (Goals, Winner, Penalty Option) before saving.");
      return;
    }
  ...
  const renderLeaderboard = () => {
    const filtered = leaderboard.filter(u => u.total_score > 0);
    if (filtered.length === 0) return <div className="p-4 border-2 border-black font-black text-center">No scores yet.</div>;
    return filtered.map((u, i) => (
      <div key={u.id} onClick={async () => { const {data} = await supabase.from('predictions').select('*').eq('user_id', u.id); setViewedUser(data); }} className="bg-white p-4 mb-2 border-2 border-black font-black flex justify-between cursor-pointer">
    ...
    <div className="min-h-screen bg-slate-50 p-6 text-black">
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {(['predictions', 'daily report', 'leaderboard', 'rules', 'matches'] as const).map(t => (
          <button key={t} onClick={() => {setActiveTab(t); setViewedUser(null)}} className={`px-4 py-2 uppercase font-black ${activeTab === t ? 'underline bg-black text-white' : 'bg-white border-2 border-black'}`}>{t}</button>
        ))}
      </div>
      ...
      {activeTab === 'predictions' && matches.map(m => (
        <div key={m.match_number} className="bg-white p-6 mb-4 border-2 border-black">
          ...
          <select value={inputs[m.match_number]?.penalties || ''} onChange={(e) => setInputs({...inputs, [m.match_number]: {...inputs[m.match_number], penalties: e.target.value}})} className="border-2 border-black p-2 w-full mt-2 text-blue-800">
            <option value="">Select Penalty Option</option>
      ...
      {activeTab === 'leaderboard' && (viewedUser ? 
        <div className="bg-white p-6 border-2 border-black">
          ...
          {viewedUser.map(p => <div key={p.id} className="border-b py-2 font-bold">{p.pred_home_goals}-{p.pred_away_goals} ({p.pred_penalties ? 'Penalties: Yes' : 'No'})</div>)}
        </div> : renderLeaderboard()
      )}
    </div>
  )
}
